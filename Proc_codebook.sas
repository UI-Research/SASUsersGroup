...
** VARIABLES SPECIFIED ON MACRO 
   Required Variables: 
        ...
        OUTFILE = name of file for the codebook created
   Optional Variables: 
...
** MODIFICATIONS: 
  ...
  8/11/2020 - Use ODS Excel instead of ODS PDF. Replace "." with "-" in seq_data to accomodate ODS Excel. Switched 
			  parameter named from pdffile to outfile. Use Lato font instead of Times. 

--------------------------------------------------------------------------------------------------------------------*/
%macro proc_codebook(lib,file1,fmtlib,outfile,include_warn=YES);
...
label name      = 'Variable Name'
      ...
      desc      = 'Category Labels'
      frequency = 'Unweighted Frequency'
      percent   = 'Unweighted Percent'
      format    = 'VAR Format';
run;
...
parent=Styles.RTF;
replace fonts /
'TitleFont' = ("Lato",10pt,Bold Italic) 
'TitleFont2' = ("Lato",10pt,Bold Italic) 
'StrongFont' = ("Lato",10pt,Bold)       
'EmphasisFont' = ("Lato",10pt,Italic)    
'headingEmphasisFont' = ("Lato",12pt,Bold Italic)
'headingFont' = ("Lato",12pt,Bold)
'docFont' = ("Lato",10pt) 
'footFont' = ("Lato",9pt) 
...
proc report data=seq_data nocenter nowindows headline wrap split='~'  missing
	...
	style(summary)=[color=very light grey backgroundcolor=very light grey fontfamily="Lato" fontsize=1pt textalign=r];;
...
define label / group flow width=30 ;  
